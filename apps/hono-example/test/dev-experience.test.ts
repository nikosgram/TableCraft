...
describe('Dev Experience — /_meta endpoint', () => {
  it('products /_meta returns column definitions', async () => {
    ...
    columns.forEach((col) => expect(col.key).toBeDefined());
  });
...
describe('Dev Experience — search', () => {
  it('search=nonexistent returns empty data array', async () => {
    const { status, json } = await get('/engine/products?search=xyznonexistent999');
    expect(status).toBe(200);
    expect(Array.isArray(json.data)).toBe(true);
  });

  it('search is case-insensitive', async () => {
    const { status: s1, json: j1 } = await get('/engine/products?search=laptop');
    const { status: s2, json: j2 } = await get('/engine/products?search=LAPTOP');
    expect(s1).toBe(200);
    expect(s2).toBe(200);
...
describe('Dev Experience — sorting', () => {
  it('sort=price returns products in ascending price order', async () => {
    const { status, json } = await get('/engine/products?sort=price&pageSize=50');
    expect(status).toBe(200);
    const prices = (json.data as Array<{ price: unknown }>).map((p) => Number(p.price));
  ...
  it('sorting by unknown field returns 400 with descriptive error', async () => {
    const { status, json } = await get('/engine/products?sort=nonexistent_field');
...
describe('Dev Experience — filters', () => {
  it('filter[category]=electronics returns only matching rows', async () => {
    const { status, json } = await get('/engine/products?filter[category]=electronics&pageSize=50');
    expect(status).toBe(200);
...
describe('Dev Experience — computed & subquery fields', () => {
  it('orders response includes computed statusLabel', async () => {
    ...
    expect(['Completed', 'Processing', 'Voided', 'Unknown']).toContain(rows[0].statusLabel as string);
    }
  });
...
describe('Dev Experience — export endpoint', () => {
  ...
  it('orders export=json returns JSON', async () => {
    const res = await fetch(`${BASE_URL}/engine/orders?export=json&pageSize=10`);
    expect(res.status).toBe(200);
