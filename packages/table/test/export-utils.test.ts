...
import { exportToCSV, exportData } from '../src/utils/export-utils';
...
function captureDownload() {
  ...
  document.body.removeChild = (node: Node) => {
    try { return origRemoveChild(node); } catch { return node; }
  };
...
describe('exportToCSV — basic', () => {
  ...
  it('applies columnMapping to headers', async () => {
    ...
    const data = [{ id: 1, name: 'Alice' }];
    exportToCSV(data, 'mapped', ['id', 'name'], { id: 'ID', name: 'Full Name' });
  ...
  it('renders null/undefined cells as empty strings', async () => {
    ...
    const data = [{ id: 1, name: null, extra: undefined }] as unknown as Array<Record<string, unknown>>;
...
describe('exportData — excel fallback', () => {
  it('returns false when exceljs is not available', async () => {
    // exceljs is an optional peer dep — not installed in test env
